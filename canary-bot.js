...
async function getInitialPrice() {
  ...
  const wsb = "wsb"
  const canary_ADDRESS = "0xb2cd91b79df296ea181aa5f6d729e5136e1853a4"
  const canary = "canary"
  ...
  let canarySGBPrice = (Number(arryPrice[1]._hex) / 10 ** 18).toFixed(5)
  let wsbUSDPrice = (sgbPrice / wsbSGBPrice).toFixed(5)
  let canaryUSDPrice = (sgbPrice / canarySGBPrice).toFixed(5)
  console.log(wsb + " USD price is " + wsbUSDPrice)
  console.log(canary + " USD price is " + canaryUSDPrice)

  clearRoles()
  lastPrice = canaryUSDPrice || 0
  let symbol = `${process.env.COIN_ID.toUpperCase()}`
  client.user.setPresence({
    activities: [{
    name: `SGB=${canarySGBPrice} ${symbol}`,
    type: `PLAYING`
    }]
  })

  arrow = mid
  client.guilds.cache.find(guild => guild.id === process.env.SERVER_ID).me.setNickname(`${symbol} ${arrow} ${process.env.CURRENCY_SYMBOL}${canaryUSDPrice}`)
